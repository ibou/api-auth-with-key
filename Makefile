...
#---PHPUNIT-#
PHPUNIT = APP_ENV=test vendor/bin/simple-phpunit
#------------#
#---------------------------------------------#
...
jwt: ## Show symfony logs.
	$(SYMFONY_CONSOLE) lexik:jwt:generate-keypair --overwrite -n --env=$(env)
.PHONY: jwt
...
sf-fixtures: ## Load fixtures.
	$(SYMFONY_CONSOLE) doctrine:fixtures:load --no-interaction --env=$(env)
.PHONY: sf-fixtures

sf-fixtures-alice: ## Load fixtures alice
	$(SYMFONY_CONSOLE) d:f:l --no-interaction --env=$(env)
.PHONY: sf-fixtures-alice
...
composer-validate: ## Validƒate composer.json file.
	$(COMPOSER) validate
...
t-filter: ## Run tests with filter (make t-filter f="testPostToCreateTreasure").
	$(PHPUNIT) --testdox --filter=$(f)
.PHONY: t-filter
